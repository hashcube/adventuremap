...
exports = Class(BottomBar, function (supr) {
	this.init = function (opts) {
		...
		this._images = [];
		if (opts.canCancel) {
			this._images.push(new Image({url: 'modules/adventuremap/images/ui/buttonClose.png'}));
		}
		...
		if (this._images.length > this._count) {
			new EditButton({
				...
				height: size - 8,
				icon: {
					image: 'modules/adventuremap/images/ui/buttonBack.png',
					x: (size - 8) * 0.2,
					y: (size - 8) * 0.18,
					width: (size - 8) * 0.6,
					height: (size - 8) * 0.6
			...
			new EditButton({
				...
				height: size - 8,
				icon: {
					image: 'modules/adventuremap/images/ui/buttonNext.png',
					x: (size - 8) * 0.2,
					y: (size - 8) * 0.18,
					width: (size - 8) * 0.6,
					height: (size - 8) * 0.6
		...
		for (var i = 0; i < this._count; i++) {
			...
			new ImageScaleView({
				...
				width: size - 16,
				height: size - 16,
				image: 'modules/adventuremap/images/ui/contentBorder.png',
				scaleMethod: '9slice',
